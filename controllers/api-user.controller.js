...
class ApiUserController {
    ...
    getUserSongs(req, res) {
        const sql = `SELECT us.name, us.singer, us.path, us.image, us.song_id AS songid, us.user_id AS userid, us.playlist_id AS playlistid FROM user_songs as us INNER JOIN users ON users.user_name = '${req.params.username}' AND users.user_id = us.user_id AND us.playlist_id = users.user_id`;
        db.query(sql, (err, result) => {
            if (err) {
                throw err;
            }
    ...
    getUserPlaylist(req, res){
        const sql = `SELECT up.playlist_id as playlistid, up.playlist_name as playlistName , count(us.song_id) AS songCount FROM user_playlists as up INNER JOIN user_songs AS us ON up.playlist_id = us.playlist_id  INNER JOIN users ON users.user_name = '${req.params.username}' AND users.user_id = up.user_id GROUP BY up.playlist_id;`
        db.query(sql, (err, result) => {
            if (err) {
